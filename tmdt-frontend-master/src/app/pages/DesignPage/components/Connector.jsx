  ...
  getConnectorPoints = stuff => {
    ...
    if (stuff.type === 'FromAndTo') {
      if (to.attrs.name.includes('rect') && from.attrs.name.includes('rect')) {
        ...
        dx =
          to.attrs.x + to.attrs.width / 2 - (from.attrs.x + fromTextWidth / 2);
        ...
        dy =
          to.attrs.y + toTextHeight / 2 - (from.attrs.y + fromTextHeight / 2);
        dx = to.attrs.x + toTextWidth / 2 - (from.attrs.x + fromTextWidth / 2);
        theta = Math.atan2(dy, dx);
        let angle = (theta / Math.PI) * 180;
        ...
        dy =
          ...
          (from.attrs.y + from.attrs.height / 2);
        dx =
          to.attrs.x + toTextWidth / 2 - (from.attrs.x + from.attrs.width / 2);
        var theta = Math.atan2(dy, dx);
        let angle = (theta / Math.PI) * 180;
